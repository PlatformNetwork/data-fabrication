"""Secure Docker CLI or broker executor for challenge-side evaluation containers."""
...
@dataclass(frozen=True)
class DockerLimits:
    """Resource limits applied to one Docker run."""
...
@dataclass(frozen=True)
class DockerRunSpec:
    """Complete Docker run specification."""
...
class DockerRunResult:
    """Result returned by a Docker evaluation run."""
...
class DockerContainerInfo:
    """Container metadata returned by the executor."""
...
class DockerExecutor:
    """Secure Docker executor compatible with the Platform SDK broker path."""
    ...
    def run(self, spec: DockerRunSpec, timeout_seconds: int) -> DockerRunResult:
        ...
        name = spec.name or self.container_name(str(spec.labels.get("platform.job", "job")))
    ...
    def cleanup_job(self, job_id: str) -> None:
        ...
        if ids:
            subprocess.run([self.docker_bin, "rm", "-f", *ids], check=False)
    ...
    def list_containers(self, job_id: str | None = None) -> list[DockerContainerInfo]:
        ...
        proc = subprocess.run(
            [self.docker_bin, "ps", "-a", *filters, "--format", "{{json .}}"],
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    def remove_container(self, name: str) -> None:
        subprocess.run([self.docker_bin, "rm", "-f", name], capture_output=True, check=False)
    ...
    with tarfile.open(fileobj=buf, mode="w") as archive:
        ...
        if source.is_dir():
            for path in source.rglob("*"):
                archive.add(path, arcname=str(path.relative_to(source.parent)))
        else:
            archive.add(source, arcname=source.name)
    return {
        "target": mount.target,
        "read_only": mount.read_only,
        "tar_base64": base64.b64encode(buf.getvalue()).decode("ascii"),
    }
